...
describe('Deferred Execution -', function () {
    ...
    it('Except() - Key', function () {
        var iterable = linq_1.default([1, 2]).Except(data_1.un2, o => o.id);
        var iterator = iterable[Symbol.iterator]();
        chai_1.assert.equal(1, iterator.next().value);
        chai_1.assert.equal(2, iterator.next().value);
        chai_1.assert.isTrue(iterator.next().done);
    ...
    it('GroupJoin() - Redundant', function () {
        var iterable = linq_1.default(data_1.un2)
            .GroupJoin(data_1.un1, e => e.id, u => u.id, (e, u) => {
            return {
                key: e.id,
                values: u
            };
        ...
        var iterator = iterable[Symbol.iterator]();
        var result = iterator.next().value;
        chai_1.assert.isTrue(Array.isArray(result.values));
        chai_1.assert.equal(result.key, 3);
        var result = iterator.next().value;
        chai_1.assert.isTrue(Array.isArray(result.values));
        chai_1.assert.equal(result.key, 4);
        chai_1.assert.isTrue(iterator.next().done);
    });
    // GroupBy
    it('GroupBy()', function () {
