...
exports.Range=Range;function*Union(first,second,keySelector){let set=new Set();for(let value of first){let key=keySelector(value);if(!key||set.has(key))
continue;set.add(key);yield value;}
for(let value of second){let key=keySelector(value);if(!key||set.has(key))
continue;set.add(key);yield value;}}
...
exports.Join=Join;function*GroupJoin(target,oKeySelect,transform,map){for(let value of target){let key=oKeySelect(value);if(!key)
continue;let innerSet=map.get(key);if(!innerSet)
...
OrderBy(keySelect,equal,generator=Generator.Forward){var comparer=equal?equal:Constant.defCompare;var compare=!keySelect&&!equal?undefined:keySelect?(a,b)=>comparer(keySelect(a),keySelect(b)):(a,b)=>comparer(a,b);return new OrderedLinq(this,(array)=>generator(array),compare);}
OrderByDescending(keySelect,equal){return this.OrderBy(keySelect,equal,Generator.Reverse);}
...
class OrderedLinq extends EnumerableImpl{constructor(target,factory,equal){super(target,factory);this.equal=equal;this['thenBy']=this.ThenBy;this['thenByDescendi']=this.ThenByDescending;}
[Symbol.iterator](){if(!this._factoryArg){this._factoryArg=this._target.ToArray();if(this.equal){this._factoryArg.sort(this.equal);}
else{this._factoryArg.sort();}}
return this._factory(this._factoryArg);}
ThenBy(keySelect,equal,generator=Generator.Forward){var comparer=equal?equal:Constant.defCompare;var compare=!keySelect&&!equal?undefined:keySelect?(a,b)=>comparer(keySelect(a),keySelect(b)):(a,b)=>comparer(a,b);if(!compare)
return this;if(!this.equal){this.equal=compare;}
else{let superEqual=this.equal;this.equal=(a,b)=>{let result=superEqual(a,b);return(0!=result)?result:compare(a,b);};}
return this;}
ThenByDescending(keySelect,equal){return this.ThenBy(keySelect,equal,Generator.Reverse);}}},{"./generators":1,"./iterators":2,"./utilities":4}],4:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports.trueFn=()=>true;exports.selfFn=(o)=>o;exports.defGrouping=(a,b)=>{if(!b[exports.CONST_KEY]){b[exports.CONST_KEY]=a;}
